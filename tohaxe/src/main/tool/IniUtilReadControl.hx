...
//#endregion
    ...
    */
    ...
    {
        var bGroup = RegexUtil.IsMatch("^\\[.+\\]", m_line);
        var bCmt   = RegexUtil.IsMatch("^\\s*;",m_line);
        var bMulti = RegexUtil.IsMatch("^\\s*.+=@@@",m_line);
        var bSingle= RegexUtil.IsMatch("^\\s*.+=.+",m_line);
        // branch
    ...
    */
    function S_GET_GROUPNAME(bFirst : Bool)
    {
        ...
        if (bFirst)
        {
            var  word = RegexUtil.Get1stMatch("\\[.+\\]",m_line);
            word = word.substr(1);
