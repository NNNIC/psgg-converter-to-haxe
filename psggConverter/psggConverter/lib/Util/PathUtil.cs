...
namespace lib.util
{
    ...
    public class PathUtil
    {
        public static string GetRelativePath(string baseUri, string targetUri)
        {
            ...
            var abs_base_uri  = _abs(baseUri);
            var abs_target_uri = _abs(targetUri);


            if (abs_base_uri.ToLower()[0] != abs_target_uri.ToLower()[0])
            {
                //ドライブが違う
                return targetUri;
            }

            if (abs_target_uri.StartsWith(abs_base_uri))
            {
                var v = abs_target_uri.Substring(abs_base_uri.Length);
            ...
            var list_base   = PathToList(abs_base_uri);
            var list_target = PathToList(abs_target_uri);

            var latestIndex = 0;
            for (var i = 0; i < list_target.Length; i++)
            {
                var base_v = ArrayUtil.GetVal(list_base, i);
                var tgt_v = ArrayUtil.GetVal(list_target, i);
            ...
            var diff = list_target.Length - latestIndex;
            //
            //
            ...
            //    newpath += list_base[i];
            //}
            for (var i = latestIndex + 1; i < list_base.Length; i++)
            {
                if (!string.IsNullOrEmpty(newpath)) newpath += @"\";
                newpath += @"..";
            }
            for (var i = latestIndex + 1; i < list_target.Length; i++)
            {
                if (!string.IsNullOrEmpty(newpath)) newpath += @"\";
